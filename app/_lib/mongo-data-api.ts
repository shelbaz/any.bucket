...
export class MongoDBDataAPI<InnerDoc = Document> {
  ...
  constructor(config: Config, baseParams?: BaseParams) {
    if (!config.apiKey && !process.env.MONGODB_DATA_API_KEY) {
...
export const createMongoDBDataAPI = (config: Config) => {
  config.apiKey = process.env.MONGODB_DATA_API_KEY || config.apiKey;
  config.appId = process.env.MONGODB_DATA_API_APP_ID || config.appId;
